...
def calculate_ndcg(tp, tn, fp, fn):
    dcg = tp / np.log2(np.arange(len(tp)) + 2)  # Example formula for DCG
    idcg = np.sort(tp)[::-1] / np.log2(np.arange(len(tp)) + 2)  # Example formula for IDCG
    return np.sum(dcg) / np.sum(idcg) if np.sum(idcg) > 0 else 0  # NDCG formula


def calculate_hit_rate(tp, fn):
    return tp / (tp + fn) if (tp + fn) > 0 else 0  # Hit Rate formula
...
def evaluation(sp, act):
    ...
    TS = (tp / (tp + fn + fp)) * 100
    y_true = np.argmax(sp, axis=1)
    y_pred = np.argmax(act, axis=1)
    # MAP = calculate_map(tp, fp, fn) * 100
    # HIT_Rate = calculate_hit_rate(tp, fn) * 100
