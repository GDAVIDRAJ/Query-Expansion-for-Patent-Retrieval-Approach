...
def Model_Proposed(Feat, sol, Target):
    Find_Singular = ['randomized', 'arpack']
    clusters = round(sol[0])
    clustering = Biclustering(n_clusters=clusters, svd_method=Find_Singular[round(sol[1])],
                              n_init=round(sol[2]))
    clustering.fit(Feat)
